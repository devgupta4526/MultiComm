...
//         return next(new ValidationError("Incorrect OTP.${ 5 - failedAttempts } attempts left."));
...
export const verifyOtp = async (email: string, otp: string, next: NextFunction) => {
    ...
    if (storedOtp !== otp) {
        ...
        await redis.set(failedAttemptsKey, failedAttempts + 1, "EX", 300);
        throw new ValidationError(`Incorrect OTP. ${5 - failedAttempts} attempts left.`);
    }
...
export const handleForgotPassword = async (req: Request, res: Response, next: NextFunction, userType: "user" | "seller") => {
    try {
        ...
        await sendOtp(user.name, email, userType === "user" ? 
            "forgot-password-user-mail" : "forgot-password-seller-mail");
        res.status(200).json({
